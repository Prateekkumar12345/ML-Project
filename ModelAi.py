...
•	try:
•	    cnx = mysql.connector.connect(host='localhost', user='root', password='Prateek@123', database='prateek12')
•	    cursor = cnx.cursor()
...
•	    try:
•	        cursor.execute("REPLACE INTO UserState (UserID, State, LastUpdated) VALUES (%s, %s, %s)", (user_id, state_json, now))
•	        cnx.commit()
•	    except Exception as e:
...
•	            response_area.insert(tk.END, f"Last conversation from {table_name}:\nYou: {last_conversation[2]}\nEmory: {last_conversation[3]}\n")
•	            response_area.see(tk.END)
•	        else:
...
•	        webbrowser.open("https://aumscn.amrita.edu/cas/login?service=https%3A%2F%2Faumscn.amrita.edu%2Faums%2FJsp%2FCore_Common%2Findex.jsp")
•	        response="opening AUMS."
•	    elif 'open amazon' in command_lower:
•	        webbrowser.open("https://www.google.com/url?sa=t&source=web&rct=j&opi=89978449&url=https://www.amazon.in/&ved=2ahUKEwjuooXFp5mFAxX5b2wGHVelB5MQFnoECAgQAQ&usg=AOvVaw11iWiBdQ4J9QF64so00kjl")
...
•	    image = Image.open("Speaking-Transparent-PNG.png")
...
•	
•	    
•	    listen_button = tk.Button(root, text="Start Listening", command=lambda: start_listening_thread(engine, user_id, response_area))
•	    listen_button.pack(pady=10)
•	   
•	    retrieve_conv_button = tk.Button(root, text="Retrieve Last Conversation", command=lambda:get_most_recent_conversation_table())
